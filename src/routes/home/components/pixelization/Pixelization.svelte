<script>
  ...
  function updatePixelated() {
    ...
    api.process(
      ...
      options.scaleX,
      options.scaleY,
      parseInt(options.distanceMode),
      parseInt(options.ditherMode),
      options.dither,
    ...
    createImageBitmap(blob).then((bitmap) => {
      ...
      if (bitmap.width > bitmap.height) {
        pixelated.style.width = `${
          bitmap.width * (original.width / original.height)
        }px`;
      }
...
>
  <h2 class="font-pixl font-extrabold text-6xl mb-4 text-center">
    {$_("pixelization.name")}
  </h2>
  <div class="flex flex-col md:flex-row w-full items-start">
    <Image on:upload={upload} />
    <Options bind:options on:updatePixelated={updatePixelated} />
  </div>
</div>
